...
const AdminSidebar: React.FC<AdminSidebarProps> = ({ current, onNavigate }) => {
  return (
    <aside className="w-64 flex flex-col bg-surface-light dark:bg-surface-dark border-r border-[#e5e7eb] dark:border-[#2d3748] h-full hidden lg:flex flex-shrink-0 transition-colors duration-200">
      <div className="flex flex-col gap-4 p-4 h-full">
        ...
        <div className="flex items-center gap-3 px-2 py-3 mt-auto border-t border-[#e5e7eb] dark:border-[#2d3748]">
          <div className="bg-center bg-no-repeat bg-cover rounded-full h-8 w-8" style={{backgroundImage: 'url("https://ui-avatars.com/api/?name=Admin")'}}></div>
          <div className="flex flex-col">
            <p className="text-sm font-medium text-[#111418] dark:text-white">Administrador</p>
            <p className="text-xs text-[#637588] dark:text-[#9ca3af]">Nível Master</p>
          </div>
        </div>
      </div>
    </aside>
